...
class LinkedList<E> implements List<E> {
  ...
  E get single {
    if (_length != 1) {
      throw StateError('Expected single element, but got $_length');
    }
    return _head!.value;
  }
  ...
  void fillRange(int start, int end, [E? fillValue]) {
    ...
    if (fillValue == null && null is! E) {
      throw ArgumentError('fillValue must not be null for non-nullable type E');
    }
    _Node<E>? current = _getNodeAt(start);
    for (int i = start; i < end; i++) {
  ...
  E firstWhere(bool Function(E element) test, {E Function()? orElse}) {
    ...
    throw StateError('No element satisfies the predicate.');
  }
  ...
  void setRange(int start, int end, Iterable<E> iterable, [int skipCount = 0]) {
    ...
    if (end - start < iterable.length - skipCount) {
      throw ArgumentError('The iterable is too long to fit in the range.');
    }
  ...
  E singleWhere(bool Function(E element) test, {E Function()? orElse}) {
    ...
    for (final element in this) {
      if (test(element)) {
        if (found) {
          throw StateError('More than one element satisfies the predicate.');
        }
        result = element;
        found = true;
      }
  ...
  Iterable<E> skip(int count) sync* {
    if (count < 0) throw ArgumentError.value(count, 'count', 'Cannot be negative');
    int skipped = 0;
    for (final element in this) {
      if (skipped >= count) {
        yield element;
