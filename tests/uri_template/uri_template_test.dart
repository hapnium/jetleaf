...
import 'package:jetleaf/uri_template.dart';
...
void main() {
  ...
  group('UriTemplate.expand', () {
    ...
    test('throws PathMatchingException for missing variable', () {
      final template = UriTemplate('/users/{id}/orders/{orderId}');
      expect(
        () => template.expand({'id': '42'}),
        throwsA(
          isA<PathMatchingException>().having(
            (e) => e.message,
            'message',
            contains('orderId'),
          ),
  ...
  group('PathMatchingException', () {
    test('has proper message and toString output', () {
      final ex = PathMatchingException('Missing segment');
