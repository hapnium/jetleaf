...
void main() {
  group('Reader', () {
    ...
    test('read throws ArgumentError on bad offset/length', () async {
      final reader = MockReader('bad');
      final buffer = List<int>.filled(5, 0);
      expect(() => reader.read(buffer, -1, 3), throwsArgumentError);
      expect(() => reader.read(buffer, 1, 10), throwsArgumentError);
    });
  });
}